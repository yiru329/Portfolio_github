...
def robot(content):  # 调用机器人
    api = r'http://api.qingyunke.com/api.php'
    # 创建post提交的数据
    data = {
        "key": apikey,
        "appid": 0,
...
def handle_response(data):
    if data.type == PROFESSIONAL_KEY:
        ...
        for message in chat_message.message:
            ...
            if _type == 1:  # 文本消息 _to == "filehelper"
                print(_from, _to, _from_group_member, content)

                if (re.search('@chatroom', _from)):  # 正则判断不是群消息
                    pass
                else:
                    m = re.search('在吗', content)
                    p = re.search('图', content)
                    if m:
                        spy.send_text(_from, "唔～,主人失联了/(ㄒoㄒ)/")
                    elif p:
                        spy.send_file(_from, r"C:\Users\BlackFlag\Desktop\wechat_bot\images\e.gif")
                    else:
                        spy.send_text(_from, robot(content))
